...
root = tk.Tk()
root.geometry("1000x400+600+300")
root.title(Title)
...
def show_overlay(duration = 2000):
    ...
    label = tk.Label(overlay, text="Adjusting for ambient noise, please wait", font=("Arial", 24), bg='grey', fg='white')
    label.place(relx=0.5, rely=0.5, anchor='center')

    # Remove the text after `duration` milliseconds
    def hide_text():
        label.destroy()
    overlay.after(duration, hide_text)


    return overlay

def translate():
    overlay = show_overlay()  # Show grey overlay
